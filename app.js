...
// ==================== تهيئة نظام الترجمة للصفحات الداخلية ====================
function initInternalPagesTranslation() {
    console.log('🔤 تهيئة الترجمة للصفحات الداخلية...');
    
    // التحقق من وجود نظام الترجمة
    if (!window.i18n) {
        console.error('❌ نظام الترجمة غير محمل');
        return;
    }
...
function initInternalPages() {
    console.log('🏢 تهيئة الصفحات الداخلية...');
    
    // تهيئة نظام المصادقة
    initInternalPagesAuth();
    
    // تهيئة نظام الترجمة
    initInternalPagesTranslation();
    
    // تحديث الواجهة بناءً على حالة المستخدم
    updateInternalPageUI();
}

// تحديث واجهة الصفحات الداخلية
function updateInternalPageUI() {
    const user = auth.currentUser;
    if (user) {
        // تحديث اسم المستخدم إذا كان موجوداً
        const nameElements = document.querySelectorAll('#student-name-display, #parent-name-display');
        nameElements.forEach(el => {
            if (el && user.displayName) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // تحميل رموز الدول
    setTimeout(() => {
        if (typeof utils !== 'undefined' && utils.loadCountryCodes) {
            utils.loadCountryCodes('qcCountryCode', '20');
        }
    }, 1000);
    ...
    function setupInstantTranslation() {
        console.log('🔄 إعداد نظام الترجمة الفوري...');
        
        // تحديث جميع العناصر عند تغيير اللغة
        document.addEventListener('languageChanged', (event) => {
            console.log('🎯 حدث تغيير اللغة - بدء التحديثات:', event.detail?.lang);
            ...
            // 4. تحديث التواصل السريع
            if (utils && utils.loadCountryCodes) {
...
// ==================== إعادة تهيئة الترجمة عند تغيير الصفحات ====================
function setupPageTransitionTranslations() {
    console.log('📄 إعداد نظام ترجمة لتحويل الصفحات...');
    
    // استمع لتغيير اللغة في جميع الصفحات
    document.addEventListener('languageChanged', (event) => {
        const lang = event.detail?.lang || currentLang();
        console.log('🌍 حدث تغيير اللغة في جميع الصفحات:', lang);
        
        // إذا كنا في صفحة داخلية، أعد تهيئة النظام
        if (!window.location.pathname.includes('index.html')) {
            if (window.i18n && window.i18n.reinitialize) {
                setTimeout(() => {
                    window.i18n.reinitialize();
                    console.log('🔄 تم إعادة تهيئة الترجمة للصفحة الداخلية');
                }, 100);
            }
        }
    });
    
    // استمع لتحميل الصفحات الداخلية
    window.addEventListener('pageshow', (event) => {
        if (!window.location.pathname.includes('index.html')) {
            console.log('📄 تحميل صفحة داخلية، إعادة تطبيق الترجمات...');
            setTimeout(() => {
                if (window.i18n && window.i18n.applyTranslations) {
                    window.i18n.applyTranslations();
                }
...
// تشغيل نظام تحويل الصفحات
setupPageTransitionTranslations();
