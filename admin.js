    ...
    function displayResultsByGroup(resultsByGroup) {
        ...
        Object.entries(resultsByGroup).forEach(([groupId, groupResults]) => {
            ...
            groupResults.forEach(result => {
                ...
                // التعديل: إزالة عرض الإيميل من الكارت الخارجي
                html += `
...
function showResultDetailsModal(userId, examId, result, student, exam) {
    ...
    modal.innerHTML = `
        <div class="modal-content-new exam-review-modal" style="max-width: 900px;">
            <div class="modal-header">
...
function createMCQuestionHTML(question, userAnswer, finalIsCorrect, originalIsCorrect, questionNumber, questionId, isManuallyCorrected = false) {
    ...
    if (question.options && Array.isArray(question.options)) {
        question.options.forEach((option, index) => {
            ...
            let statusIcon = '';
            if (isUserChoice && isCorrectOption) {
                statusIcon = '<i class="fas fa-check-circle option-status correct"></i>';
            } else if (isUserChoice && !isCorrectOption) {
                statusIcon = '<i class="fas fa-times-circle option-status wrong"></i>';
            } else if (!isUserChoice && isCorrectOption) {
                statusIcon = '<i class="fas fa-star option-status correct-star"></i>';
            }
            ...
            optionsHTML += `
                <div class="${optionClass}">
                    ...
                    ${isUserChoice ? '<span class="user-choice-label">إجابتك</span>' : ''}
                    ${isCorrectOption ? '<span class="correct-label">الإجابة الصحيحة</span>' : ''}
                </div>
            `;
        });
    }
...
function openUserModal(key = null, user = null) {
    ...
    const studentAssignmentFieldHTML = (user?.role === 'parent' || (isNew && 'role' === 'parent')) ? `
        <div class="full" id="student-assignment-field">
    ...
    // إضافة CSS واحد فقط - لا تضيف أنماط متعددة
    ...
    style.textContent = `
        ...
        /* تصميم حقل تعيين الطلاب */
        .student-assignment-wrapper {
            display: flex;
        ...
        .student-select:focus {
            outline: none;
            box-shadow: 0 0 0 3px rgba(var(--bg-text-rgb), 0.1);
        ...
        .selected-student-preview i {
            font-size: 2rem;
            color: var(--bg-text);
        }
        
        .selected-student-preview div {
            flex: 1;
        }
        
        .selected-student-preview strong {
            display: block;
            color: var(--bg-text);
            font-size: 1.1rem;
        ...
        .selected-student-preview small {
            color: var(--bg-text);
            opacity: 0.8;
            font-size: 0.9rem;
        }
    ...
    // تحميل خيارات الطلاب إذا كان المستخدم ولي أمر
    if (user?.role === 'parent' && studentAssignmentField) {
        loadStudentOptions(modal, user?.assignedStudent || '');
    }
    
    // إظهار/إخفاء حقل تعيين الطالب عند تغيير الدور
...
// دالة لتحميل الطلاب وتعيينهم في القائمة
function loadStudentOptions(modal, selectedStudentId = '') {
    ...
    // إظهار/إخفاء حقل تعيين الطالب بناءً على الدور
    ...
    if (roleSelect && studentAssignmentField) {
        roleSelect.addEventListener('change', function() {
            if (this.value === 'parent') {
                studentAssignmentField.style.display = 'block';
                // تحميل الطلاب إذا لم يتم تحميلهم
